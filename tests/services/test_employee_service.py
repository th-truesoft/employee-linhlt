...
@pytest.fixture
def employee_service(db_session: Session):
    """Create EmployeeService instance with dependency injection"""
    repository = EmployeeRepository(Employee)
    return EmployeeService(repository)


def test_search_employees_no_filters(
    db_session: Session, test_data, employee_service: EmployeeService
):
...
def test_search_employees_with_name_filter(
    db_session: Session, test_data, employee_service: EmployeeService
):
...
def test_search_employees_with_department_filter(
    db_session: Session, test_data, employee_service: EmployeeService
):
...
def test_search_employees_with_position_filter(
    db_session: Session, test_data, employee_service: EmployeeService
):
    if not test_data["employees"]:
        pytest.skip("Employee data is empty")
...
def test_search_employees_with_location_filter(
    db_session: Session, test_data, employee_service: EmployeeService
):
    if not test_data["employees"]:
        pytest.skip("Employee data is empty")
...
def test_search_employees_with_pagination(
    db_session: Session, test_data, employee_service: EmployeeService
):
    if not test_data["employees"]:
        pytest.skip("Employee data is empty")
...
def test_search_employees_with_custom_columns(
    db_session: Session, test_data, employee_service: EmployeeService
):
    if not test_data["employees"]:
        pytest.skip("Employee data is empty")
