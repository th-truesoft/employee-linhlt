...
class RateLimiter:
    """
    A simple in-memory rate limiter that uses a token bucket algorithm with multi-tenant support.
    ...
    def _get_client_id(self, request: Request) -> str:
        ...
        if user_id:
            return f"{client_ip}:{organization_id}:{user_id}"

        return f"{client_ip}:{organization_id}"
...
rate_limiter = RateLimiter(
    ...
    window_size=getattr(
        settings, "RATE_LIMIT_WINDOW_SIZE", 60
